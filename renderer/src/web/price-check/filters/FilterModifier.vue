<template>
  <div :class="$style['filter']" class="group">
    ...
    <div class="flex flex-col min-w-0 flex-1">
      <div class="pb-px flex items-baseline justify-between">
        <button
          ...
          type="button"
        >
          <span class="w-5 flex justify-start" style="transform: translateY(2px)">
            <span class="ee-check" :class="{ 'ee-check--on': !isDisabled }" />
          </span>
          <div
...
.filter {
  @apply py-2 px-1 -mx-1 rounded-md;
...
.rollInput {
  ...
  &:hover {
    border-color: rgba(228, 190, 138, 0.35);
  }

  &:focus {
    border-color: theme("colors.accent.500");
    box-shadow: 0 0 0 1px rgba(228, 190, 138, 0.4);
    cursor: none;
  }
}
...
.qualityLabel {
  ...
  border: 1px solid rgba(228, 190, 138, 0.3);
  background: rgba(228, 190, 138, 0.08);
  @apply rounded;
  @apply px-2;
  @apply text-center;
}
