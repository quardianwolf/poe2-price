...
export interface Config {
  ...
  // Live data updates: pull regenerated game data from a repo when a patch lands
  dataUpdate: {
    repo: string; // "owner/repo" on GitHub hosting renderer/public/data
    branch: string;
    auto: boolean; // check & apply automatically on launch
  };
}
...
function upgradeConfig(_config: Config): Config {
  ...
  if (config.configVersion < 35) {
    config.dataUpdate = defaultConfig().dataUpdate;
