  ...
  >
    <div class="widget-default-style">
      <div class="p-1 flex gap-1 items-center text-base">
        <template v-for="widget in widgets" :key="widget.wmId">
          <button
            ...
            :class="
              widget.wmWants === 'show'
                ? 'border-accent-500/70 text-accent-100 shadow-glow'
                : 'border-gray-700 text-gray-200 hover:border-gray-600'
            "
            class="bg-gray-800 rounded-md p-2 leading-none whitespace-nowrap border transition-all"
          >
            <i
              v-if="widget.icon"
      ...
      <div v-else class="px-1 pb-1">
        <textarea
          class="px-2 py-1.5 bg-gray-900 text-gray-200 rounded-md resize-none block w-full border border-white/5 placeholder:text-gray-600 focus:border-accent-500/60 transition-colors"
          rows="1"
          spellcheck="false"
